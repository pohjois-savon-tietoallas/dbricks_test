...
```{r, eval = eval_mining}
readLines("./data/arthur_doyle.txt", 30)
```
...
0. Moving data files to dbfs

    ```{r, eval = eval_mining}
    # fs::file_copy("./data/arthur_doyle.txt", "/dbfs/arthur_doyle.txt")
    # fs::file_copy("./data/mark_twain.txt", "/dbfs/mark_twain.txt", overwrite = T)
    system("cp ./data/arthur_doyle.txt /dbfs/arthur_doyle.txt")
    system("cp ./data/mark_twain.txt /dbfs/mark_twain.txt")
    list.files("/dbfs/")
    ```


1. Load `SparkR` and open new session
    ```{r, eval = eval_mining}
    library(SparkR)
    sparkR.session()
    ```

2. Load the `sparklyr` library and open a Spark session
    ```{r, eval = eval_mining}
    library(sparklyr)
...
4. Use the `spark_read_text()` function to read the **arthur_doyle.txt** file, assign it to a variable called `doyle`
    ```{r, eval = eval_mining}
    doyle <- spark_read_text(sc, "doyle", "arthur_doyle.txt") 
    ```
